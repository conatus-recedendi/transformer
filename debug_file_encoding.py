...
def main():
    ...
    def analyze_file(file_path, name):
        ...
        with open(file_path, "rb") as f:
            ...
            newline_byte = b"\n"
            crlf_bytes = b"\r\n"
            print(f"  Ends with \\n: {tail_bytes.endswith(newline_byte)}")
            print(f"  Ends with \\r\\n: {tail_bytes.endswith(crlf_bytes)}")
        ...
        with open(file_path, "rb") as f:
            ...
            lf_byte = b"\n"
            crlf_seq = b"\r\n"
            null_count = data.count(null_byte)
            cr_count = data.count(cr_byte)
            lf_count = data.count(lf_byte)
            crlf_count = data.count(crlf_seq)
    ...
    print(
        f"Python line count diff: {abs(en_stats['python_utf8'] - de_stats['python_utf8']):,}"
    )
